...
def invoice_list(invoices: list, page: int = 0, per_page: int = 5) -> InlineKeyboardMarkup:
    ...
    for inv in invoices[start:start+per_page]:
        ...
        kb.row(InlineKeyboardButton(text=f"{S.get(sv,'📄')} {inv.invoice_number}  •  {inv.total:,.0f} ت", callback_data=f"inv:view:{inv.id}"))
    nav = []
    if page > 0: nav.append(InlineKeyboardButton(text="◀️", callback_data=f"inv:list:{page-1}"))
    nav.append(InlineKeyboardButton(text=f"📄 {page+1}", callback_data="noop"))
    if start+per_page < len(invoices): nav.append(InlineKeyboardButton(text="▶️", callback_data=f"inv:list:{page+1}"))
    if nav: kb.row(*nav)
...
def invoice_detail(inv_id: int, status: str, inv_number: str) -> InlineKeyboardMarkup:
    ...
    if status in ("draft","sent","partial"):
        kb.row(InlineKeyboardButton(text="💳 پرداخت", callback_data=f"inv:pay:{inv_id}"),
               InlineKeyboardButton(text="📤 ارسال شد", callback_data=f"inv:send:{inv_id}"))
    kb.row(InlineKeyboardButton(text="📄 PDF", callback_data=f"inv:pdf:{inv_id}"),
           InlineKeyboardButton(text="✅ پرداخت کامل", callback_data=f"inv:markpaid:{inv_id}"))
    kb.row(InlineKeyboardButton(text="📋 کپی شماره فاکتور", copy_text=CopyTextButton(text=inv_number)))
...
def client_list(clients: list, page: int = 0, per_page: int = 7) -> InlineKeyboardMarkup:
    ...
    nav = []
    if page > 0: nav.append(InlineKeyboardButton(text="◀️", callback_data=f"cli:list:{page-1}"))
    if start+per_page < len(clients): nav.append(InlineKeyboardButton(text="▶️", callback_data=f"cli:list:{page+1}"))
    if nav: kb.row(*nav)
...
def client_detail(cid: int, phone: str = "") -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="📄 فاکتورها", callback_data=f"cli:invoices:{cid}"),
           InlineKeyboardButton(text="🏗 پروژه‌ها", callback_data=f"cli:projects:{cid}"))
    if phone:
        kb.row(InlineKeyboardButton(text="📞 کپی تلفن", copy_text=CopyTextButton(text=phone)))
    kb.row(InlineKeyboardButton(text="✏️ ویرایش", callback_data=f"cli:edit:{cid}"),
...
def project_list(projects: list, page: int = 0, per_page: int = 5) -> InlineKeyboardMarkup:
    ...
    nav = []
    if page > 0: nav.append(InlineKeyboardButton(text="◀️", callback_data=f"prj:list:{page-1}"))
    if start+per_page < len(projects): nav.append(InlineKeyboardButton(text="▶️", callback_data=f"prj:list:{page+1}"))
    if nav: kb.row(*nav)
...
def project_detail(pid: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="📄 فاکتورها", callback_data=f"prj:invoices:{pid}"),
           InlineKeyboardButton(text="💸 هزینه‌ها", callback_data=f"prj:expenses:{pid}"))
    kb.row(InlineKeyboardButton(text="👷 کارگران", callback_data=f"prj:workers:{pid}"),
           InlineKeyboardButton(text="📊 گزارش PDF", callback_data=f"prj:report:{pid}"))
    kb.row(InlineKeyboardButton(text="✏️ ویرایش", callback_data=f"prj:edit:{pid}"),
...
def worker_list(workers: list, page: int = 0, per_page: int = 7) -> InlineKeyboardMarkup:
    ...
    for w in workers[start:start+per_page]:
        ...
        kb.row(InlineKeyboardButton(text=f"{icon} {w.name}  •  {w.skill or 'عمومی'}", callback_data=f"wrk:view:{w.id}"))
    nav = []
    if page > 0: nav.append(InlineKeyboardButton(text="◀️", callback_data=f"wrk:list:{page-1}"))
    if start+per_page < len(workers): nav.append(InlineKeyboardButton(text="▶️", callback_data=f"wrk:list:{page+1}"))
    if nav: kb.row(*nav)
...
def worker_detail(wid: int, phone: str = "") -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="📋 ثبت کارکرد", callback_data=f"wrk:log:{wid}"),
           InlineKeyboardButton(text="💳 پرداخت", callback_data=f"wrk:pay:{wid}"))
    if phone:
        kb.row(InlineKeyboardButton(text="📞 کپی تلفن", copy_text=CopyTextButton(text=phone)))
    kb.row(InlineKeyboardButton(text="✏️ ویرایش", callback_data=f"wrk:edit:{wid}"),
...
def finance_menu() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="💵 دریافتی‌ها", callback_data="fin:income"),
           InlineKeyboardButton(text="💸 هزینه‌ها", callback_data="fin:expenses"))
    kb.row(InlineKeyboardButton(text="📊 سود/زیان", callback_data="fin:profit"),
...
def report_menu() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="📊 خلاصه مالی", callback_data="rep:summary"),
           InlineKeyboardButton(text="📄 فاکتورها", callback_data="rep:invoices"))
    kb.row(InlineKeyboardButton(text="🏗 پروژه‌ها", callback_data="rep:projects"),
           InlineKeyboardButton(text="👷 کارگران", callback_data="rep:workers"))
    kb.row(InlineKeyboardButton(text="📅 ماهانه", callback_data="rep:monthly"),
...
def confirm_kb(action: str, item_id: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="✅ بله، حذف شود", callback_data=f"confirm:{action}:{item_id}"),
           InlineKeyboardButton(text="❌ خیر", callback_data=f"cancel:{action}:{item_id}"))
    return kb.as_markup()

def add_more_items() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="➕ اقلام بیشتر", callback_data="inv:addmore"),
