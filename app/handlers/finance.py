...
async def fin_income(cb: CallbackQuery, session: AsyncSession):
    ...
    await cb.message.edit_text(
        f"💵 <b>دریافتی‌ها</b>\n\n💰 جمع کل: <b>{fmt(total)}</b>\n✅ فاکتور پرداخت شده: <b>{len(paid_invs)}</b> عدد",
...
async def fin_expenses(cb: CallbackQuery, session: AsyncSession):
    ...
    text = f"💸 <b>هزینه‌ها</b>\n\nجمع کل: <b>{fmt(total)}</b>\n\n<b>آخرین هزینه‌ها:</b>\n"
    for e in recent: text += f"• {e.category} | {fmt(e.amount)}\n"
...
async def fin_profit(cb: CallbackQuery, session: AsyncSession):
    ...
    await cb.message.edit_text(
        f"{icon} <b>سود / زیان</b>\n\n💵 درآمد: <b>{fmt(income)}</b>\n💸 هزینه: <b>{fmt(expense)}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n{icon} <b>{'سود' if profit >= 0 else 'زیان'}: {fmt(abs(profit))}</b>",
        reply_markup=finance_menu()
    )
    await cb.answer()
...
async def fin_debtors(cb: CallbackQuery, session: AsyncSession):
    ...
    r = await session.execute(select(Invoice).options(selectinload(Invoice.items), selectinload(Invoice.payments), selectinload(Invoice.client)).where(Invoice.status.in_(["draft","sent","partial"])))
    invs = r.scalars().all()
    debtors = sorted([(inv, inv.remaining) for inv in invs if inv.remaining > 0], key=lambda x: x[1], reverse=True)
    ...
    if not debtors: text += "✅ هیچ بدهی‌ای وجود ندارد!"
    await cb.message.edit_text(text, reply_markup=finance_menu())
    await cb.answer()
